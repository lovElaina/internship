...
const Index: React.FC = () => {
  ...
  return (
    ((currentUser?.user?.roleId == 1) ||(currentUser?.user?.roleId) == 3 ||(currentUser?.user?.roleId) == 5) ? (
      <WrapContent>
        <Card>
          <Alert
          ...
          <Typography.Title level={2} style={{ textAlign: 'center' }}>
            <SmileTwoTone /> 欢迎您！{!loading && (currentUser?.user?.roles[0].roleName + " "+currentUser?.user?.userName)}
      ...
      </WrapContent>) : (currentUser?.user?.roleId) == 4 ? <Workplace info={currentUser}/> : <div>loading...</div>
  );
};
